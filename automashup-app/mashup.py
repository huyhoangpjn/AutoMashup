...
def new_mashup_technic(tracks):
    sr = tracks[0]['sr'] # The first track is used to determine the bpm
    tempo= tracks[0]['metadata']["bpm"]

    beginning_temporal = tracks[0]['metadata']['downbeats'][0]
    ...
    for track in tracks:
        ...
        track_tempo = track['metadata']["bpm"]

        track_beginning_temporal = track['metadata']["downbeats"][0]
...
def mashup_technic_fit_phase(tracks):
    ...
    return new_mashup_technic(tracks)
